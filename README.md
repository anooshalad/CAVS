...
- apps/api — FastAPI backend that orchestrates file storage, OCR (EasyOCR), PDF rendering (PyMuPDF / fitz), field extraction, and validation checks.
...
2. Install the likely required packages (example requirements — adapt versions if needed):
   ...
   ```powershell
   pip install fastapi uvicorn[standard] python-multipart easyocr pymupdf
   ```
...
2. By default the frontend will try to call http://localhost:8000. To change the API base URL, create a Vite env variable in a .env file:

   ```text
   VITE_API_BASE_URL=http://localhost:8000
   ```

3. Open the dev URL printed by Vite (typically http://localhost:5173) and use the UI to upload files and analyze.
...
- The frontend uses Vite for a fast dev experience. The frontend expects an API base URL in VITE_API_BASE_URL, falling back to http://localhost:8000.
